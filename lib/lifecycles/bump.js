  ...
  return runLifecycleScript(args, 'prerelease')
    ...
    .then(release => {
      if (!args.firstRelease) {
        ...
        newVersion = args.target
          ? semver.valid(args.target)
          : semver.valid(releaseType) ||
              semver.inc(pkg.version, releaseType, args.prerelease)
...
Bump.pkgFiles = ['package.json', 'bower.json', 'manifest.json']

Bump.lockFiles = ['package-lock.json', 'npm-shrinkwrap.json']
  ...
  Object.keys(configsToUpdate).forEach(function (configPath) {
    try {
      ...
      if (stat.isFile()) {
        ...
        checkpoint(args, 'bumping version in ' + filename + ' from %s to %s', [
          config.version,
          newVersion
        ])
